...
class RPDO implements Driver
{
	...
	protected function setEncoding()
	{
		...
		if ($driver === 'mysql') {
			...
			$this->pdo->setAttribute(\PDO::MYSQL_ATTR_INIT_COMMAND, 'SET NAMES '. $charset ); //on every re-connection
	...
	public function connect()
	{
		...
		try {
			...
			throw new \PDOException( 'Could not connection to database (' . $dbname . ').', $exception->getCode() );
		}
	}
...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	*/
	 ...
	 * This method can be used to quickly connection beans using indirect
	 * relations. For instance, given an album and a song you can connection the two
 ...
 *
 * An adapter class to connection various database systems to RedBean
...
class SimpleFacadeBeanHelper implements BeanHelper
{
	 ...
	 * to connection a bean to a model.
	 *
 ...
 *
 * RedBeanPHP FUSE is a mechanism to connection beans to posthoc
...
class SimpleModel
{
	...
	/**
	 * Used by FUSE: the ModelHelper class to connection a bean to a model.
...
class Facade
{
	 ...
	 * Instead, it will prepare the connection and connection 'lazily', i.e. the moment
